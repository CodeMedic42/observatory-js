...
function load(observable, controlMember) {
    let __Control;
    ...
    function _emit(force) {
        ...
        if (this._status.deepChangePending != null) {
            // Deep change
            this.emit('change:deep', this._status.deepChangePending);

            this._status.deepChangePending = null;
        }

        if (this._status.shallowChangePending) {
            this._status.shallowChangePending = false;
    ...
    function _linkControl(name, control, shallowLink) {
        const content = this._content[name];
        content.control = control;

        this._value[name] = content.control._value;

        this._status.deepChangePending = Symbol();
        this._status.shallowChangePending = shallowLink;

        let changeListener = (instance) => {
            if (instance === this._status.deepChangePending) {
                return;
            }

            this._value[name] = content.control._value;

            this._status.deepChangePending = instance;

            _emit.call(this);
        };

        let replaceListener = (replacement) => {
            content.disenguage(false);

            _linkControl.call(this, name, replacement, false);
        };

        content.disenguage = (shallowDisenguage) => {
            this._status.shallowChangePending = shallowDisenguage;

            content.control.removeListener('change:deep', changeListener);
        ...
        content.control.on('change:deep', changeListener);
    ...
    function _setupObject(item, name) {
        ...
        if (_.isNil(content)) {
            ...
            if (item.control instanceof __Control) {
                ...
                control = item[controlMember].control;
            } else {
                control = __Control(item);
            }

            _linkControl.call(this, name, control, true);
        } else {
    ...
    function _setAsArray(newData) {
        while (this._content.length > newData.length) {
            this._content.pop().disenguage(true);
        }
    ...
    function _setAsSimple(newData) {
        ...
        this._value = newData;
        this._status.deepChangePending = Symbol();
        this._status.shallowChangePending = true;
    }
    ...
    function _addProperty(id, value) {
        ...
        if (value instanceof __Control) {
            _linkControl.call(this, id, value, true);
        } else {
            _linkControl.call(this, id, __Control(value), true);
        }
    ...
    function _delete(id, use) {
        this._content[id].disenguage(true);
    ...
    __Control = function Control(...args) {
        ...
        this._status = {
            deepChangePending: null,
            shallowChangePending: false
        };
